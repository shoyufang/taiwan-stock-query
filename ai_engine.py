"""
Agnes AI 引擎 — Phase 7 (取代 Gemini)
使用 openai SDK 連線至 AI API，提供智能查詢與分析
"""
...
class AIEngine:
    ...
    def __init__(self, api_key: str, model_name: str = ""):
        ...
        try:
            ...
            cfg = load_config()
            base_url = cfg.get("ai_base_url", _DEFAULT_BASE_URL) or _DEFAULT_BASE_URL
            self.client = OpenAI(
                api_key=api_key,
                base_url=base_url
            )
            main_logger.info(f"AI Engine 初始化成功: {self.model_name} (base_url={base_url})")
        except Exception as e:
            main_logger.error(f"AI Engine 初始化失敗: {str(e)}")
            self.client = None
    ...
    def list_available_models(api_key: str) -> List[str]:
        ...
        try:
            ...
            main_logger.info("正在從 AI API 獲取可用模型清單...")
            models = client.models.list()
            model_ids = sorted([m.id for m in models.data])
            if not model_ids:
                return AIEngine.FALLBACK_MODELS[:]
            return model_ids
        except Exception as e:
            main_logger.error(f"獲取可用模型清單失敗: {str(e)}")
            return AIEngine.FALLBACK_MODELS[:]

    def smart_query(self, user_input: str) -> Dict[str, Any]:
        """執行智能查詢 (自動函式調用)"""
        if not self.client:
            return {"error": "AI API Key 或模型未設定"}

        main_logger.info(f"[AI SMART QUERY] 用戶輸入: {user_input}")
        ...
        try:
            ...
            if tool_calls:
                ...
                for tool_call in tool_calls:
                    ...
                    main_logger.info(f"AI 調用工具: {function_name}，參數: {function_args}")
            ...
            err_str = str(e)
            main_logger.error(f"AI 執行查詢失敗: {err_str}")
    ...
    def summarize_news(self, news_text: str, subject: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            main_logger.error(f"AI 新聞摘要失敗: {str(e)}")
            return {"error": str(e)}
...
def get_ai_engine() -> "Optional[AIEngine]":
    ...
    api_key = get_secret("AGNES_API_KEY") or get_secret("ai_api_key")
    model_name = get_secret("AGNES_MODEL") or get_secret("ai_model")
    if api_key:
        return AIEngine(api_key, model_name=model_name)
    return None
...
def generate_us_stock_report(ticker: str) -> str:
    """
    獲取個股所有美股數據，呼叫 AI 引擎生成一鍵 AI 健檢與投資研究報告
    """
    engine = get_deepseek_engine()
    if not engine or not engine.client:
        return "⚠️ 未偵測到有效的 AI API Key，請前往『設定』進行配置。"
...
# 向後相容別名：舊程式碼 import DeepSeekEngine 仍能運作
DeepSeekEngine = AIEngine
