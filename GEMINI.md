...
- [x] **台股選股引擎與三大法人端點修復 (2026-05-21)**：
    ...
    - [x] **台股法說與除息日曆擴充實作完成**：
      - [x] **台股50強與日程解析引擎**：新建 `tw_calendar.py`，定義 50 檔最具代表性的台股權值巨頭（台灣 50 指數成分股）。自動提取法說會/財報日期、下季預估 EPS、下季預估營收（台幣元）及除息日。
      - [x] **本地中文名稱映射**：串聯 `stock_lookup.py` 機制，將台股代號自動轉換為 100% 正確的中文名稱。
...
- [x] **Phase 4 app.py 拆分實作完成 (2026-06-10)**：
    - [x] **獨立分頁模組化**：完成將 app.py 中所有的 tab UI 渲染函數全數拆分至 `tabs/` 目錄中，並新建 `dispatch.py` 作為中樞分發路由，有效防止循環導入。
    - [x] **極致精簡主程式**：[app.py](file:///g:/%E6%88%91%E7%9A%84%E9%9B%B2%E7%AB%AF%E7%A1%AC%E7%A2%9F/Claude%E5%B0%88%E5%8D%80/%E6%B0%B8%E8%B1%90%E9%87%91API/app.py) 規模從 3,162 行縮減至 316 行，降低程式碼耦合度，提高可維護性。
    - [x] **測試 Mock 路徑修復**：修正單元測試中的 mock 導入路徑，確保與模組化後的架構 100% 相容。
    - [x] **282項測試全綠**：本機 `pytest` 測試 100% 綠燈通過。
- [ ] 待命：等待使用者回饋或新功能開發指令。
